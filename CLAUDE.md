...
- Each push has its own subfolder in `drafts/`, named after the version it will become — `drafts/v1.0/`, then `drafts/v1.1/`, and so on — holding that version's draft file (`draft-v1.0.md`) and its map (`draft-v1.0-map.html`) together. The active draft is the one for the next version to be pushed (currently [drafts/v0.1.1/draft-v0.1.1.md](drafts/v0.1.1/draft-v0.1.1.md)). [drafts/v0.1/draft-v0.1.md](drafts/v0.1/draft-v0.1.md) was pushed as v0.1 on 2026-09-22 and is finished; fixes and additions found while building and reviewing its prototype go into draft-v0.1.1.md instead of reopening it. [drafts/v0.2/draft-v0.2.md](drafts/v0.2/draft-v0.2.md) and [drafts/v1.0/draft-v1.0.md](drafts/v1.0/draft-v1.0.md) stay pending. Drafts are never combined into one document.
- Draft entries have no ID. Note the type, the likely home parent, and any other parents it also serves, as suggestions only.
- **Every draft always includes the full text of every objective and problem** — a new-scope draft (like draft-v0.2), a patch draft (like draft-v0.1.1), and every future draft, without exception. Never a compact ID-only list. Before the first push there's no objectives.md/problems.md yet, so the draft itself is the working copy of their full text; once they exist, later drafts mirror the full text from objectives.md/problems.md rather than rewriting it, noting that those files remain the source of truth to edit.
...
- **A draft's .md file and its .map.html must never contradict each other.** Any change to a draft — adding, editing, or removing an entry or a relationship, including the full objectives and problems required above — is applied to both the text and the map in the same pass, no exceptions. Before finishing any change to a draft, check that every item named in the text appears as a node in the map and every item in the map has a matching entry in the text, and that every home-parent and also-serves relationship stated in the text has the matching link in the map (solid for home, dashed for also-serves) and no other links exist beyond those. Same items, same relationships, both places, always.
...
- The active draft's map lives beside it, in the same version subfolder, as `draft-v<version>-map.html`. On a push, copy the map into `versions/v<version>/mindmap.html` alongside the document snapshots.
- **A new draft that is a sequel to an existing one keeps its diff highlighted, permanently.** When a brand-new draft file is created as the next version of a draft that already exists (pushed or not — draft-v0.1 → draft-v0.1.1, draft-v0.1 → draft-v0.2, and so on), build its mind map using the same highlighted-diff treatment as a mirroring preview map (see "Mirroring an update into a later draft" below): every new item highlighted and labelled by type, every amended item (an "Amends" entry) highlighted and labelled as amended, and every new link highlighted, distinguishing a new home-parent link from a new "also serves" link, and calling out any link that connects two items that already existed. This highlighted version *is* that draft's real, permanent mind map — not a temporary preview, and nothing gets stripped back to a plain version afterward. Browsing the maps version by version then shows, at each step, exactly what was added or changed, preserving that history and context. This does not apply retroactively on its own — an existing draft's map already built the plain way stays as it is unless the user asks for it to be rebuilt this way too.
